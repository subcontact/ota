/*** Generated by streamline 0.10.4 (generators) - DO NOT EDIT ***/

var galaxy = require("streamline/lib/generators/runtime"); 
(galaxy.unstar(function*(_) {
  var getFolders_ = galaxy.unstar(getFolders, 2), 
  getFiles_ = galaxy.unstar(getFiles, 2), 
  getBuildProjects_ = galaxy.unstar(getBuildProjects, 1), 
  getBuildProjectList_ = galaxy.unstar(getBuildProjectList, 1), 
  getProjectsService_ = galaxy.unstar(getProjectsService, 2), 
  getProjectBuildListService_ = galaxy.unstar(getProjectBuildListService, 1), 
  getProjectBuildDataService_ = galaxy.unstar(getProjectBuildDataService, 1),
   main_ = galaxy.unstar(main, 0); 
"use strict";
...
function* getFolders(dirPath, filters, _) {
  ...
  var folders = [];
  var files   = (yield galaxy.invoke(fs, "readdir", [dirPath, _], 1));
  if (filters && filters.name) {
    nameFilter = filters.name;
  }
  for (var i=0; i< files.length; i++) {
    var fullFile = path.normalize(dirPath + '/' + files[i]);
    stat = (yield galaxy.invoke(fs, "stat", [fullFile, _], 1));
    if (stat.isDirectory()) {
      if (nameFilter) {
...
function* getBuildProjects(rootPath, _) {
  ...
  var buildProjects = (yield getFolders(rootPath, null, _));
...
var fileExists = galaxy.streamlinify(function(path, cb) {
...
function* getBuildProjectList(buildProfilePath, _) {
    ...
    if ((yield galaxy.invoke(null, fileExists, [fullFile, _], 1))) {
      var stat = (yield galaxy.invoke(fs, "stat", [fullFile, _], 1));
      if (stat.isDirectory()) {
        var buildList = (yield getFolders(fullFile, { name : BUILD_LIST_PATTERN }, _)); // using direct regex syntax rather than a string 
      }
    }
    return buildList;
}
...
function* getProjectsService(meta, p, _) {
  ...
  var buildProjects   = (yield getBuildProjects(p, _));
  ...
  for (var i=0; i<meta.buildProjects.length; i++) {
    list = (yield getBuildProjectList(meta.buildProjects[i].path, _));
    if (list.length > 0) {
      dirPath = list[0];
      buildInfo = (yield galaxy.invoke(null, findBuildFile, [dirPath, _], 1));
      if (buildInfo) {
...
function* getProjectBuildListService(project, _) {
  ...
  list = (yield getBuildProjectList(project.path, _));
  project.list = [];
  for (var i=0; i<list.length; i++) {
    ...
    buildMeta = (yield galaxy.invoke(null, findBuildFile, [list[i], _], 1));
    if (buildMeta) {
...
function* main(_) {
  try {
    ...
    console.log('###A###');
    result = (yield getProjectsService(meta, p, _));
    console.log(util.inspect(result, { showHidden: true, depth: null }));

    console.log('###B###');
    result = (yield getProjectBuildListService(meta.buildProjects[0], _));
    console.log(util.inspect(result, { showHidden: true, depth: null }));

    console.log('###C###');
    result = (yield getProjectBuildDataService(meta.buildProjects[0].list[0], _));
    console.log(util.inspect(result, { showHidden: true, depth: null }));
...
(yield main(_));}, 0).call(this, function(err) {
  if (err) throw err;
}));